...
binary_main=grayImage>graythresh(grayImage);
% blobMeasurements_max = regionprops(de_main_one, grayImage, 'all');%ABB

%%%figure;imshow(binary_main)
...
binary_main=gray_dering>graythresh(gray_dering); % this is to define the shape
newb=imfill(binary_main,'holes');
% de_main_one=bwareafilt(binary_main,1);
blobMeasurements_all = regionprops(binary_main, grayImage, 'all');%ABB
% blobMeasurements_max = regionprops(de_main_one, grayImage, 'all');%ABB

% MajorAxisLength=blobMeasurements_max(1).MajorAxisLength;
% MinorAxisLength=blobMeasurements_max(1).MinorAxisLength;
% 
% min2max_ratio(i,1)=MinorAxisLength/MajorAxisLength;
gray_dering=imgaussfilt(dering_img,0.7);
dering_binary=rem_simcan(gray_dering,20/255);
figure;imshow(binary_main)
blobMeasurements_all = regionprops(dering_binary, grayImage, 'all');%ABB

a=extractfield(blobMeasurements_all,'Area')';
a_m=sort(a,'descend');
ind_area=length(a);
num_main=sum(isoutlier(a_m));
if length(a_m==1)
    num_main=1;
end
    
dering_outm=bwareafilt(dering_binary,num_main);
%figure;imshow(dering_outm)
dering_binary=dering_outm;
dering_img=dering_outm.*grayImage;
% % var=std(a_m(1:5));
...
imwrite(grayImage,['/home/hhuan006/dering/twoway/ori' num2str(i) '.bmp'])
imwrite(ori_binary,['/home/hhuan006/dering/twoway/bi_ori' num2str(i) '.bmp'])
imwrite(dering_img,['/home/hhuan006/dering/twoway/der' num2str(i) '.bmp'])
imwrite(dering_binary,['/home/hhuan006/dering/twoway/bi_der' num2str(i) '.bmp'])
