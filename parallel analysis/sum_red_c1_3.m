...
tic
clear; close all; clc
path='/home/hhuan006/RedSea_analysis/comb/SPACE/T=29/sub500/cast-1/';
% path='/home/hhuan006/comb_2021fall/SPACE/T=29/sub500/cast-1/';
...
%% preprocess the depth data , making them bin to every 1 m.
depth_ava=ceil(m_big{:,15});
id_ava=find(depth_ava>0);
m_big{:,15}=depth_ava;
m_big=m_big(id_ava,:);
%% down-cast (duplicate depth in two-way cast)

depth_bin=m_big{:,15};
idx_max=max(find(depth_bin==max(depth_bin)));
m_big_down=m_big(1:idx_max,:);
depth_bin=m_big_down{:,15};
...
ctd_rest_perim=m_big_down{:,7:end};
...
name_col={'Depth(m)','Particle no.','Average particle volume','Total particle volume','Date','Time','pressure','temperature', 'salinity', 'fluorescent','oxygen','PAR','potential temperature','density', 'OBS','Beam attenuation','Transmission','voltage','hour'};
...
ori_ctd_bin=csvread(['/home/hhuan006/CTD_redsea/bin/' cs{2} '.csv'],1,0);
miss_dep=[];
dp_ctd=(ori_ctd_bin(:,9));
idx_max_dp_ctd=find(dp_ctd==max(dp_ctd));
ctd_down=ori_ctd_bin(1:idx_max_dp_ctd(end),:);
dp_down_bin=ctd_down(:,9);

m_bin_down=m_bin_ori_down;
dp_particle=m_bin_down{:,1};
name_col={'Depth(m)','Particle no.','Average particle volume','Total particle volume','Date','Time','pressure','temperature', 'salinity', 'fluorescent','oxygen','PAR','potential temperature','density', 'OBS','Beam attenuation','Transmission','voltage','hour'};
name_col_1={'Depth(m)','Particle no.','Average particle volume','Total particle volume','Date','Time'};
name_col_2={'pressure','temperature', 'salinity', 'fluorescent','oxygen','PAR','potential temperature','density'};
name_col_3={'OBS','Beam attenuation','Transmission','voltage','hour'};

if dp_particle(length(dp_particle))-dp_particle(1)+1~=length(dp_particle)
    for i = 1:length(dp_down_bin)-1
        dp_particle=m_bin_down{:,1};
        if dp_particle(i)~=dp_particle(i+1)-1
            
            miss_dep=[miss_dep;dp_particle(i)+1];
            
            point_ms=dp_particle(i)+1;
            id_ms_ctd=find(dp_down_bin==point_ms);
            id_ms_img=min(find(dp_particle==dp_particle(i)));
            pre=m_bin_down(1:id_ms_img,:);
            post=m_bin_down(id_ms_img+1:height(m_bin_down),:);
            column_ms=[array2table([point_ms,NaN,NaN,NaN,NaN,NaN],'VariableNames',name_col_1),array2table(ctd_down(id_ms_ctd,1:8),'VariableNames',name_col_2),array2table(ctd_down(id_ms_ctd,10:end),'VariableNames',name_col_3)];
...
m_big_up=m_big(idx_max+1:end,:);
depth_bin=m_big_up{:,15};
depth_idx=findgroups(depth_bin);
depth_group_uni=unique(depth_idx);
depth_bin_uni=unique(depth_bin);
...
dp_ctd=ori_ctd_bin(:,9);
idx_max_dp_ctd=find(dp_ctd==max(dp_ctd));
dp_ctd_up=dp_ctd(idx_max_dp_ctd+1:end);
id_nzero=find(dp_ctd_up~=0);
ctd_up=ori_ctd_bin(idx_max_dp_ctd+1:end,:);
ctd_up=ctd_up(id_nzero,:);
dp_ctd_up=dp_ctd_up(id_nzero,:);
...
if dp(1)~=max(dp_ctd)-1
   for i = 1:max(dp_ctd)-dp(1)-1 
       ...
       t=renamevars(array2table(ctd_up(id_ms_ctd,11:end)),1:4,name_col_3);
       column_ms=[array2table([ctd_up(id_ms_ctd,10),NaN,NaN,NaN,NaN,NaN],'VariableNames',name_col_1),array2table(ctd_up(id_ms_ctd,1:9),'VariableNames',name_col_2),t];
...
if dp(length(dp))-dp(1)+1~=length(dp)
    for i = 1:length(dp_ctd_up)-1
        ...
        if dp(i)~=dp(i+1)+1
            ...
            t=renamevars(array2table(ctd_up(id_ms_ctd,11:end)),1:4,name_col_3);
            column_ms=[array2table([point_ms,NaN,NaN,NaN,NaN,NaN],'VariableNames',name_col_1),array2table(ctd_up(id_ms_ctd,1:9),'VariableNames',name_col_2),t];
...
save_path='/home/hhuan006/RedSea_analysis/sum/raw/T=29/';
% save_path='/home/hhuan006/sum_2021fall/all/T=29/sub500/';
