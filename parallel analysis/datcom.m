...
clear all;
    close all;
    myFolder = ('./2021fall/diff2021fall/cast-2/');
    if ~isdir(myFolder)
    ...
    if (sec-1)*each>=length(theFiles)
        writetable(cell2table(M),[savepath 'T=7_date_time-cast-2_sec=5.csv']); 
    else
...
M=sortrows(M,1);
L=cell2table(M);
savepath='/home/hhuan006/batch_2021fall/SPACE/T=7/sub500/cast-2/';
if ~exist(savepath)
  mkdir(savepath)
end
writetable(L,[savepath 'T=7_date_time-cast-2_sec=5.csv']); 
    end
...
path= '/home/hhuan006/batch_2021fall/SPACE/T=7/sub500/cast-2/';
path_ctd='/home/hhuan006/CTD2021fall/';
ctd_list=dir(fullfile(path_ctd,'*cast-2.*'));
ctd_name=ctd_list(1).name;
data_ctd=csvread(strcat(path_ctd,ctd_name),1,0);
filelist=dir(fullfile(path,'*cast-2_sec=5.csv'));
n=1;
Comb={};
m=1;
...
L=cell2table(Comb);
save_path='/home/hhuan006/comb_2021fall/SPACE/T=7/sub500/cast-2/';
if ~exist(save_path)
  mkdir(save_path)
end
